...
public class Puzzle extends AbstractPuzzle {
   ...
   public static <T> Set<T[]> getPermutationsRecursive( T[] num ) {
      if ( num == null ) { return null; }

      Set<T[]> perms = new HashSet<>();

      //base case
      if ( num.length == 0 ) {
         perms.add( Arrays.copyOf( num, 0 ) );
         return perms;
      }
      ...
      T first = num[0];
      T[] remainder = Arrays.copyOfRange( num, 1, num.length );
      Set<T[]> subPerms = getPermutationsRecursive( remainder );
      for ( T[] subPerm : subPerms ) {
         for ( int i = 0; i <= subPerm.length; ++i ) { // '<='   IMPORTANT !!!
            T[] newPerm = Arrays.copyOf( subPerm, subPerm.length + 1 );
